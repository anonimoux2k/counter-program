...
pub fn process_instruction(
    ...
    match instruction {
        ...
        CounterInstructions::Decrement(args) => {
            let subtracted_value = if args.value > counter_account.counter {
                counter_account.counter
            } else {
                args.value
            };
            counter_account.counter -= subtracted_value;
        }
...
mod test {
    ...
    fn test_counter() {
        ...
        let mut decrement_instruction_data_2nd: Vec<u8> = vec![1];
        let mut update_instruction_data: Vec<u8> = vec![2];
        let reset_instruction_data: Vec<u8> = vec![3];

        let additional_value = 20u32;
        increment_instruction_data.extend_from_slice(&additional_value.to_le_bytes());
        ...
        let subtraced_value = 5u32;
        decrement_instruction_data.extend_from_slice(&subtraced_value.to_le_bytes());
        ...
        let subtraced_value = 16u32;
        decrement_instruction_data_2nd.extend_from_slice(&subtraced_value.to_le_bytes());
        process_instruction(&program_id, &accounts, &decrement_instruction_data_2nd).unwrap();
